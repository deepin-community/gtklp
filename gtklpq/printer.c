...
void printerSetDefaults (int t)
{
  ...
  gtk_list_select_item (GTK_LIST (GTK_COMBO (printerFrameCombo[t])->list),
			prt);
  /* PrinterInfo */
  gtk_label_set_text (GTK_LABEL (printerFrameInfoText[t]), str2str(PrinterInfos[prt]));
  /* PrinterLocation */
  gtk_label_set_text (GTK_LABEL (printerFrameLocationText[t]), str2str(PrinterLocations[prt]));
	      ...
	      GdkEventButton * event, gpointer data)
{
  ...
  gtk_widget_set_sensitive ((GtkWidget *) buttonCancel, TRUE);
	...
	if(strcmp((char *)text,str2str(_("H"))) == 0)
	{
				...
				gtk_tooltips_set_tip(tooltips, buttonHold, str2str(_("Release Job")), str2str(_("Release Job")));
	...
	else
	{
				...
				gtk_tooltips_set_tip(tooltips, buttonHold, str2str(_("Hold Job")), str2str(_("Hold Job")));
	}	
...
void printQueueInit (GtkWidget * container)
{
  ...
  /* Queue List */
	...
	for(i=0; i<7; i++) {
		...
		gtk_clist_set_column_justification(GTK_CLIST(queueList),i,GTK_JUSTIFY_LEFT);
	}
...
#if GTK_MAJOR_VERSION == 1
	gtk_clist_set_column_width (GTK_CLIST (queueList), 1, max(GTKLPQ_RANK_SIZE,gdk_string_width(gtk_widget_get_style(titles[1])->font,str2str(_("Rank")))));
	gtk_clist_set_column_width (GTK_CLIST (queueList), 2, max(GTKLPQ_OWNER_SIZE,gdk_string_width(gtk_widget_get_style(titles[2])->font,str2str(_("Owner")))));
	gtk_clist_set_column_width (GTK_CLIST (queueList), 3, max(GTKLPQ_JOB_SIZE,gdk_string_width(gtk_widget_get_style(titles[3])->font,str2str(_("Job")))));
	gtk_clist_set_column_width (GTK_CLIST (queueList), 4, max(GTKLPQ_FILE_SIZE,gdk_string_width(gtk_widget_get_style(titles[4])->font,str2str(_("File(s)")))));
#else
	gtk_clist_set_column_width (GTK_CLIST (queueList), 1, max(GTKLPQ_RANK_SIZE,gdk_string_width(gtk_style_get_font(gtk_widget_get_style(titles[1])),str2str(_("Rank")))));
	gtk_clist_set_column_width (GTK_CLIST (queueList), 2, max(GTKLPQ_OWNER_SIZE,gdk_string_width(gtk_style_get_font(gtk_widget_get_style(titles[2])),str2str(_("Owner")))));
	gtk_clist_set_column_width (GTK_CLIST (queueList), 3, max(GTKLPQ_JOB_SIZE,gdk_string_width(gtk_style_get_font(gtk_widget_get_style(titles[3])),str2str(_("Job")))));
	gtk_clist_set_column_width (GTK_CLIST (queueList), 4, max(GTKLPQ_FILE_SIZE,gdk_string_width(gtk_style_get_font(gtk_widget_get_style(titles[4])),str2str(_("File(s)")))));
#endif
	  ...
	  != NULL)
	{
	  ...
	  switch (ippGetInteger (attr, 0))
	    ...
	    case IPP_PRINTER_PROCESSING:
	      strncpy (toolstring, str2str(_("Printer Processing")),(size_t)MAXLINE);
	      break;
	    case IPP_PRINTER_STOPPED:
	      strncpy (toolstring, str2str(_("Printer Stopped")),(size_t)MAXLINE);
				aktPrinterHold=1;
	      break;
	    default:
		...
		if(nox == 0) {
			if(aktPrinterHold==1) {
				...
				gtk_tooltips_set_tip(tooltips, buttonHoldPrinter, str2str(_("Start Printer")), str2str(_("Start Printer")));
			}
			else {
				gtk_container_remove(GTK_CONTAINER(buttonHoldPrinter),printerimage);
				...
				gtk_tooltips_set_tip(tooltips, buttonHoldPrinter, str2str(_("Stop Printer")), str2str(_("Stop Printer")));
			}

      if(jobsRejected==1) {
        gtk_container_remove(GTK_CONTAINER(buttonRejectJob),rejectjobimage);
        rejectjobimage=gtk_pixmap_new(gtklpq_accept_job,mgtklpq_accept_job);
        gtk_widget_show(rejectjobimage);
        gtk_container_add(GTK_CONTAINER(buttonRejectJob),rejectjobimage);

        gtk_tooltips_set_tip(tooltips, buttonRejectJob, str2str(_("Accept Jobs")), str2str(_("Accept Jobs")));
      }
      else {
        gtk_container_remove(GTK_CONTAINER(buttonRejectJob),rejectjobimage);
        rejectjobimage=gtk_pixmap_new(gtklpq_reject_job,mgtklpq_reject_job);
        gtk_widget_show(rejectjobimage);
        gtk_container_add(GTK_CONTAINER(buttonRejectJob),rejectjobimage);

        gtk_tooltips_set_tip(tooltips, buttonRejectJob, str2str(_("Reject Jobs")), str2str(_("Reject Jobs")));
      }
		}
	}
	else
...
int printQueueUpdate (GtkWidget * widget, gpointer data)
{
  ...
  if (strlen (ServerAddr) + strlen (PrinterNames[PrinterChoice] + 20) < MAX_URI)
    {
			z1=strchr(PrinterNames[PrinterChoice],'/');
			if(z1 != NULL)
				*z1=(uintptr_t)NULL;		
			snprintf (resource, (size_t)MAX_URI, "ipp://%s/printers/%s", ServerAddr, PrinterNames[PrinterChoice]);
			if(z1 != NULL)
				*z1='/';
  ...
  if ((response = cupsDoRequest (cupsHttp, request, "/")) != NULL)
	...
	else
	{
		...
		if(nox == 1) {
			...
			l5=max(strlen(str2str(_("Job(s)"))),73-l1-l2-l3-l4-l6-l7);	
    ...
    if (strcmp (ippGetName (attr), "copies") == 0 && ippGetValueTag (attr) == IPP_TAG_INTEGER)
	  ...
	  switch (i1)
	    ...
	    case IPP_JOB_PROCESSING:	/* Active 5 */
	      strncpy (queryLine[0], str2str(_("A")),(size_t)MAXLINE);
	      break;
	    case IPP_JOB_HELD:	/* Hold 4 */
	      strncpy (queryLine[0], str2str(_("H")),(size_t)MAXLINE);
	      break;
	    case IPP_JOB_PENDING:	/* Pending 3 */
	      strncpy (queryLine[0], str2str(_("P")),(size_t)MAXLINE);
	      break;
	    case IPP_JOB_STOPPED:
	      strncpy (queryLine[0], str2str(_("S")),(size_t)MAXLINE);
	      break;
	    case IPP_JOB_CANCELLED:
	      strncpy (queryLine[0], str2str(_("ca")),(size_t)MAXLINE);
	      break;
	    case IPP_JOB_ABORTED:
	      strncpy (queryLine[0], str2str(_("ab")),(size_t)MAXLINE);
	      break;
	    case IPP_JOB_COMPLETED:
	      strncpy (queryLine[0], str2str(_(".")),(size_t)MAXLINE);
	      break;
	    default:
	  ...
	  strncpy (queryLine[1], str2str(rankstr),(size_t)MAXLINE);
	  strncpy (queryLine[2], str2str((char *)jobuser),(size_t)MAXLINE);
	  snprintf (queryLine[3], (size_t)MAXLINE, "%d", jobid);
	  strncpy (queryLine[4], str2str((char *)jobname),(size_t)MAXLINE);
    ...
    {
			if(nox == 0)
	      m_id_upd = gtk_statusbar_push ((GtkStatusbar *) statbar, c_id_upd, str2str((char *)ippErrorString (cupsLastError ())));
			else
				g_print("[%s] %s: %s\n\n",PrinterNames[PrinterChoice], str2str(_("Error")), ippErrorString (cupsLastError ()));
      return(FALSE);
    }
...
{
	strncpy(reason,(char *)gtk_entry_get_text(GTK_ENTRY(entry)),(size_t)MAXLINE);
}
...
void queueFunc (GtkWidget * widget, gpointer data)
{
...
#else
	qfwin = gtk_dialog_new_with_buttons(str2str(_("Are you sure?")),GTK_WINDOW(mainWindow),GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,GTK_STOCK_YES,1,GTK_STOCK_NO,0,NULL);
  ...
  gtk_widget_show (label);
	...
	if (strcmp ((char *) data, "MOVE") == 0)	
	{
		...
		PrinterFrame(vbox,1);
#if GTK_MAJOR_VERSION != 1
		gtk_signal_connect_object (GTK_OBJECT(GTK_COMBO(printerFrameCombo[1])->entry), "activate",GTK_SIGNAL_FUNC(queueFunc_response_yes),NULL);
#endif
	}
...
#if GTK_MAJOR_VERSION != 1
    gtk_signal_connect_object (GTK_OBJECT(prioSpin), "activate",GTK_SIGNAL_FUNC(queueFunc_response_yes),NULL);
#endif

  }

	if (((strcmp ((char *) data, "HOLDPRINTER") == 0) && (aktPrinterHold == 0)) || ((strcmp ((char *) data, "REJECTJOB") == 0) && (jobsRejected == 0)))
	{
		...
		gtk_signal_connect(GTK_OBJECT(entry), "changed", GTK_SIGNAL_FUNC (reasonChanged), NULL);
		gtk_widget_show(entry);

#if GTK_MAJOR_VERSION != 1
    gtk_signal_connect_object (GTK_OBJECT(entry), "activate",GTK_SIGNAL_FUNC(queueFunc_response_no),NULL);
#endif
  ...
  if (strcmp ((char *) data, "CANCEL") == 0)
    {
			cancelmode=0;
      gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to cancel this job?")));
#if GTK_MAJOR_VERSION == 1
	...
	{
			cancelmode=1;
			gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to cancel ALL jobs?")));
#if GTK_MAJOR_VERSION == 1
			gtk_signal_connect_object (GTK_OBJECT (yes), "clicked", GTK_SIGNAL_FUNC (cancelJob), GTK_OBJECT (qfwin));
#endif
	}
	if (strcmp ((char *) data, "HOLD") == 0)
	{
			...
			{
				cancelmode=2;
				gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to hold this job?")));
#if GTK_MAJOR_VERSION == 1
				gtk_signal_connect_object (GTK_OBJECT (yes), "clicked", GTK_SIGNAL_FUNC (cancelJob), GTK_OBJECT (qfwin));
#endif
			}
			...
			{
				cancelmode=3;
				gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to release this job?")));
#if GTK_MAJOR_VERSION == 1
				gtk_signal_connect_object (GTK_OBJECT (yes), "clicked", GTK_SIGNAL_FUNC (cancelJob), GTK_OBJECT (qfwin));
#endif
			}
	...
	if (strcmp ((char *) data, "MOVE") == 0)
	{
		...
		gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to move this Job to the following queue?")));
#if GTK_MAJOR_VERSION == 1
		gtk_signal_connect_object (GTK_OBJECT (yes), "clicked", GTK_SIGNAL_FUNC (cancelJob), GTK_OBJECT (qfwin));
#endif
	}
	if (strcmp ((char *) data, "HOLDPRINTER") == 0)
	{
		...
		{
			cancelmode=5;
			gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to stop this printer?")));
#if GTK_MAJOR_VERSION == 1
			gtk_signal_connect_object (GTK_OBJECT (yes), "clicked", GTK_SIGNAL_FUNC (cancelJob), GTK_OBJECT (qfwin));
#endif
		}
		...
		{
			cancelmode=6;
			gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to start this printer?")));
#if GTK_MAJOR_VERSION == 1
			gtk_signal_connect_object (GTK_OBJECT (yes), "clicked", GTK_SIGNAL_FUNC (cancelJob), GTK_OBJECT (qfwin));
#endif
		}
	...
	{
		cancelmode=7;
		gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to change the job priority?")));
#if GTK_MAJOR_VERSION == 1
  ...
  if (strcmp ((char *) data, "REJECTJOB") == 0)
  {
    ...
    {
      cancelmode=8;
      gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to reject jobs for this printer?")));
#if GTK_MAJOR_VERSION == 1
      gtk_signal_connect_object (GTK_OBJECT (yes), "clicked", GTK_SIGNAL_FUNC (cancelJob), GTK_OBJECT (qfwin));
#endif
    }
    ...
    {
      cancelmode=9;
      gtk_label_set_text (GTK_LABEL (label), str2str(_("Do you really want to accept jobs for this printer?")));
#if GTK_MAJOR_VERSION == 1
      gtk_signal_connect_object (GTK_OBJECT (yes), "clicked", GTK_SIGNAL_FUNC (cancelJob), GTK_OBJECT (qfwin));
#endif
    }
...
int jobAttr(int job_id, int num_options, cups_option_t *options)
{
  ...
  ippSetRequestId(request, 1);	
	...
	{
		snprintf (uri, (size_t)MAX_URI, "ipp://%s/jobs/%i", ServerAddr, job_id);
		if(DEBUG)
			printf("uri: %s\n",uri);
	}
	else
...
void cancelJob (GtkWidget * dialog)
{
   ...
   */
    ...
    case 8:	/* reject jobs */
      op = CUPS_REJECT_JOBS;
      job_id=0;
      if(DEBUG) printf("IPP_REJECT_JOBS\n");
    break;
    case 9:	/* accept jobs */
      op = CUPS_ACCEPT_JOBS;
      job_id=0;
      if(DEBUG) printf("IPP_ACCEPT_JOBS\n");
    break;
  ...
  if ((strlen (ServerAddr) +  25) < MAX_URI)
  {
		...
		if(z2 != NULL)
			*z2=(uintptr_t)NULL;	
		snprintf (uri, (size_t)MAX_URI, "ipp://%s/printers/%s", ServerAddr, PrinterNames[PrinterChoice]);
		snprintf (touri,(size_t)MAX_URI, "ipp://%s/printers/%s", ServerAddr, PrinterNames[PrtDlgSel]);
		if(z1 != NULL)
			*z1='/';
		if(z2 != NULL)
			*z2='/';
	...
	if ((op == IPP_PURGE_JOBS) || (op == IPP_PAUSE_PRINTER) || (op == IPP_RESUME_PRINTER) || (op == CUPS_ACCEPT_JOBS) || (op == CUPS_REJECT_JOBS))
	{
	...
	if (response != NULL)
	...
	else
	  {
			if(op == IPP_PURGE_JOBS) {
				quick_message(str2str(_("Unable to cancel job(s)!")),2);
			} else {
			if(op == IPP_PAUSE_PRINTER) {
				quick_message(str2str(_("Unable to cancel printer!")),2);
			} else {
			if(op == IPP_RESUME_PRINTER) {
				quick_message(str2str(_("Unable to start printer!")),2);
			} else {
				if(op == CUPS_ACCEPT_JOBS) {
				quick_message(str2str(_("Unable to accept jobs for this printer!")),2);
			} else {
				if(op == CUPS_REJECT_JOBS) {
				quick_message(str2str(_("Unable to reject jobs for this printer!")),2);	
			} else {
				quick_message(str2str(_("Unable to do so!")),2);
			}}}}}
	    return;
