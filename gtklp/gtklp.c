...
int mainWindowHeight, mainWindowWidth, mainWindowX, mainWindowY, wantSaveSizePos;
#endif
GtkWidget *dialog;
...
int main(int argc,char *argv[])
{
  ...
  commandline_opts=NULL;
	...
	if(nox == 0) {
		...
		// XXXX gtk_widget_show(mainWindow);
		...
		gtk_box_pack_start(GTK_BOX(mainVBox), progressBar, TRUE, TRUE, 0);
	...
	while(i1 != -1)
	{
		switch(i1)
		{
			case 'H':
				...
				{
					if(optarg == (char *)NULL) {
						g_print("-H: %s\n",str2str(_("This option requires an value !")));
						break;
					}
					if(strcmp(optarg,"hold") == 0) {
						num_commandline_opts=cupsAddOption("job-hold-until", "indefinite", num_commandline_opts,&commandline_opts);	
						break;
					}
					if((strcmp(optarg,"resume") == 0) || (strcmp(optarg,"release") == 0)) {
						num_commandline_opts=cupsAddOption("job-hold-until", "no-hold", num_commandline_opts,&commandline_opts);
						break;
					}
					if(strcmp(optarg,"immediate") == 0) {
						num_commandline_opts=cupsAddOption("job-priority", "100", num_commandline_opts,&commandline_opts);
						break;
					}
					num_commandline_opts=cupsAddOption("job-hold-until", optarg, num_commandline_opts,&commandline_opts);
			...
			case 'i':
				...
				if(calledLp)
				{
					g_print("lp -i: %s \n",str2str(_("Function is not supported for now !")));
					exit(-1);
					break;
				}
				if(calledLpr)
			...
			case 'p':
				if(calledLpr)
				{
					num_commandline_opts=cupsParseOptions("prettyprint=true",num_commandline_opts,&commandline_opts);
					break;	
				}
				if(calledLp)
					break;
			...
			case 'P':
				if(calledLp)
				{
					num_commandline_opts=cupsAddOption("page-ranges", optarg, num_commandline_opts,&commandline_opts);
			...
			case 'l':
				...
				if(calledLpr)
				{
					num_commandline_opts=cupsParseOptions("raw=true",num_commandline_opts,&commandline_opts);
				}
			break;
			case 'D':
				DEBUG=1;
			...
			break;
			case 'V':
				g_print("\n%s %s %s\n",PROGNAME,str2str(_("version")),VERSION);
				g_print("%s\n\n",MY_HOME);
				return(0);
			break;
			case 'o':
			...
			case 'q':
				if(atoi(optarg) < 0) {
					num_commandline_opts=cupsAddOption("job-priority", "0", num_commandline_opts,&commandline_opts);
					break;
				}
				if(atoi(optarg) > 100) {
					num_commandline_opts=cupsAddOption("job-priority", "100", num_commandline_opts,&commandline_opts);
				...
				num_commandline_opts=cupsAddOption("job-priority", optarg, num_commandline_opts,&commandline_opts);
			break;
			case 's':
				if(calledLp)
				{
					if(DEBUG)
						printf("Silent mode not necessary yet. I AM silent...\n");
					break;
				}
				if(calledLpr)
					break;
...
#else
				exitOnError(str2str(_("-E: %s\n")),str2str(_("Sorry, this function is not compiled in !")),-2);
#endif
			break;
			default:
				if(DEBUG)
	...
	if(progressBar != (GtkWidget *)NULL)
		gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(progressBar),0.025);
#endif
	filesToPrintAnz=0;
    		if (optind < argc)
        {
                while (optind < argc)
		{
			...
			{
				strncpy(filesToPrint[(unsigned char)filesToPrintAnz++],argv[optind++],(size_t)MAXPATH);
			}
			else
			{
				if(DEBUG)
...
#if GTK_MAJOR_VERSION != 1
	if(progressBar != (GtkWidget *)NULL)
		gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(progressBar),0.05);
#endif
  ...
  /*-- GUI Init ende ---*/
...
#if GTK_MAJOR_VERSION != 1
	if(progressBar != (GtkWidget *)NULL)
		gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(progressBar),0.075);
#endif
    ...
    gtk_box_pack_start(GTK_BOX(buttonBox), buttonPrint, FALSE, TRUE, 0);
    gtk_widget_show (buttonPrint);

#if GTK_MAJOR_VERSION != 1
		buttonReset = gtk_button_new_from_stock(GTK_STOCK_REVERT_TO_SAVED);
#else
		buttonReset = gtk_button_new_with_label (str2str(_("Reset All")));
#endif
    ...
    gtk_widget_show (buttonAbbruch);
...
#if GTK_MAJOR_VERSION != 1
		if(progressBar != (GtkWidget *)NULL) {
			gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(progressBar),0.95);
			while (gtk_events_pending())
				gtk_main_iteration();
		}
#endif
...
#if GTK_MAJOR_VERSION != 1
		if(progressBar != (GtkWidget *)NULL) {
			gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(progressBar),0.99);
			while (gtk_events_pending())
				gtk_main_iteration();
		}
#endif
 	   ...
 	   if(wantSaveSizePos == 1) {
  	    if(mainWindowHeight > 0 && mainWindowWidth > 0) {
    	    ...
    	    gtk_widget_set_uposition(GTK_WIDGET(mainWindow), mainWindowX, mainWindowY);
					while (gtk_events_pending())
						gtk_main_iteration();
	        gtk_window_resize(GTK_WINDOW(mainWindow), mainWindowWidth, mainWindowHeight);
					while (gtk_events_pending())
						gtk_main_iteration();
      	}
 	   }
